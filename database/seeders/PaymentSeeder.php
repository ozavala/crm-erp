...
class PaymentSeeder extends Seeder
{
    ...
    private function createPaymentsForPayable($payable): void
    {
        ...
        if (rand(0, 2) == 0) { // ~33% chance of full payment
            Payment::factory()->create(['payable_id' => $payable->getKey(), 'payable_type' => get_class($payable), 'amount' => $totalAmount, 'created_by_user_id' => $payable->created_by_user_id]);
        } else { // ~66% chance of partial payment
            $partialAmount = round($totalAmount * rand(20, 80) / 100, 2);
            if ($partialAmount > 0) {
                Payment::factory()->create(['payable_id' => $payable->getKey(), 'payable_type' => get_class($payable), 'amount' => $partialAmount, 'created_by_user_id' => $payable->created_by_user_id]);
            }
        }
    }
}