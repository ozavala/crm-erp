...
<div class="row">
    <div class="col-md-4 mb-3">
        <label for="entry_date" class="form-label">{{ __('journal_entries.Entry Date') }} <span class="text-danger">*</span></label>
    ...
    <div class="col-md-4 mb-3">
        <label for="transaction_type" class="form-label">{{ __('journal_entries.Transaction Type') }} <span class="text-danger">*</span></label>
        <select class="form-select @error('transaction_type') is-invalid @enderror" id="transaction_type" name="transaction_type" required>
            <option value="">{{ __('journal_entries.Select Type') }}</option>
    ...
    <div class="col-md-4 mb-3">
        <label for="description" class="form-label">{{ __('journal_entries.Description') }}</label>
...
<hr>
<h4>{{ __('journal_entries.Entry Lines') }}</h4>
    ...
    @foreach($currentLines as $index => $line)
        ...
        <div class="row align-items-center mb-2 journal-line-row border p-2 rounded">
            <div class="col-md-5 mb-2">
                ...
                <input type="text" name="lines[{{ $index }}][account_name]" class="form-control @error('lines.'.$index.'.account_name') is-invalid @enderror" value="{{ $line['account_name'] ?? '' }}" placeholder="{{ __('journal_entries.e.g., Cash, Office Expense') }}" required>
...
<button type="button" id="add-line-btn" class="btn btn-success btn-sm mt-2">{{ __('journal_entries.Add Line') }}</button>

<div class="row mt-3">
    <div class="col-md-5 offset-md-5">
        <div class="d-flex justify-content-between">
            <strong>{{ __('journal_entries.Total Debits:') }}</strong> <span id="total-debits">$0.00</span>
        </div>
        <div class="d-flex justify-content-between">
            <strong>{{ __('journal_entries.Total Credits:') }}</strong> <span id="total-credits">$0.00</span>
        </div>
         <div class="d-flex justify-content-between mt-1" id="balance-status">
            <strong>{{ __('journal_entries.Balance:') }}</strong> <span class="text-danger">{{ __('journal_entries.Out of Balance') }}</span>
        </div>
    </div>
</div>

<div class="mt-4">
    <button type="submit" class="btn btn-primary">{{ isset($journalEntry->journal_entry_id) && $journalEntry->journal_entry_id ? __('journal_entries.Update Journal Entry') : __('journal_entries.Create Journal Entry') }}</button>
    <a href="{{ route('journal-entries.index') }}" class="btn btn-secondary">{{ __('journal_entries.Cancel') }}</a>
</div>
